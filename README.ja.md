...
VSMemo は Markdown メモの作成、表の編集、ファイル整理を支援する Visual Studio Code 拡張機能です。サイドバーには最近使ったメモ・固定したメモ・移動先を表示し、編集コマンドはコマンドパレットとエディタの操作メニューから使えます。
...
- **メモのサイドバー**: 最近・固定したメモを開き、設定済みのメモ保存先を検索し、移動先を確認できます。
...
アクティビティバーの VSMemo には「メモ」と「移動先」の2つのビューがあります。メモには設定済み保存先で最近開いた Markdown ファイルを最大10件表示し、行の操作で固定できます。見出しから作成・検索を実行できます。検索は実行時に保存先を走査し、見出し・ファイル名・パスで絞り込めます。移動先ビューには現在の対象と設定済みフォルダを表示します。行の選択だけでは移動せず、行の移動ボタンで実行します。エディタ右上の VSMemo ボタンには、カーソル位置と編集可否に合う操作が表示されます。
...
1. コマンドパレット (`Ctrl+Shift+P` または `Cmd+Shift+P`)、**VSMemo** サイドバー、またはエディタの VSMemo 操作ボタンを開きます。
2. 以下のコマンドを実行します：
